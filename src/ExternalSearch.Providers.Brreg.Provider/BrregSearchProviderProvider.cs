...
{
    public BrregSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(
    ...
    {
        if (jobData is BrregExternalSearchJobData result) return await Task.FromResult(result.ToDictionary());

        throw new InvalidOperationException(
            $"Unexpected data type for {nameof(BrregExternalSearchJobData)}, {jobData.GetType()}");
    }
