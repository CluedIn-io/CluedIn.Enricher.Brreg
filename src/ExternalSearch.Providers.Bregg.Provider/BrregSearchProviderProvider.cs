...
{
    public class BrregSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public BrregSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new BrregExternalSearchJobData(configuration);
        ...
        {
            if (jobData is BrregExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(BrregExternalSearchJobData)}, {jobData.GetType()}");
        }
        ...
        public Dictionary<string, object> ExtraInfo { get; } = new()
        {
            ...
            { "nameKeyField", Constants.KeyName.NameVocabularyKey },
            { "vocabKeyPrefix", BrregVocabulary.Organization.KeyPrefix},
        ...
        {
            {
                ...
                new HashSet<string>
                {
                    Constants.KeyName.NameVocabularyKey,
                    Constants.KeyName.CountryCodeVocabularyKey,
                    Constants.KeyName.WebsiteVocabularyKey
                }
            },
            {
                "Brreg Code",
                new HashSet<string>
                {
                    Constants.KeyName.BrregCodeVocabularyKey
                }
            }
        };
    }
